...
const BuyProduct = () => {
  ...
  const handlePurchase = async () => {
    if (!canPurchase) {
      alert('Hanya buyer yang dapat melakukan pembelian. Silakan login sebagai buyer.');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/purchases`, {
        method: 'POST',
        headers: {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              {canPurchase ? (
                <>
                  <div className="mb-6">
                    <label className="block text-gray-700 mb-2">Jumlah (Ton)</label>
                    <input 
                      type="number" 
                      min="1" 
                  ...
                  <div className="mb-4">
                    ...
                    <p><span className="font-medium">Jumlah:</span> {quantity} Ton</p>
                    <p><span className="font-medium">Harga Per Ton:</span> Rp {product.hargaPerTon?.toLocaleString()}</p>
                    <p className="text-xl font-bold mt-2">Total: Rp {(product.hargaPerTon * quantity).toLocaleString()}</p>
                  </div>
